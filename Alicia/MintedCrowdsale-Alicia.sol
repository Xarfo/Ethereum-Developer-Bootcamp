...
contract MintedCrowdsale is YourToken{  //always inherit import contract
    // Attach SafeMath library functions to the uint256 type.
    using SafeMath for uint256;//library for a data type

    // Define 4 publicly accessible state variables.
    // Your custom token being sold.
    YourToken public token;//own data type - use sparingly
    ...
    * 4) Amount of tokens purchased (amount)
    */
    ...
    */
    ...
    //fallback functions - external- can be called from outside contract
    function buyTokens(address _beneficiary) public payable {
        ...
        weiRaised = weiRaised.add(weiAmount);//SafeMath library being used here - prevents overflow attacks
        ...
        _forwardFunds();//openzepplin has crowdsale validation process - extendable
    }
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal {
        ...
        require(_beneficiary != address(0), "Check the beneficiary's address");
        require(_weiAmount != 0, "Check the value wei raised");
    }
    ...
    function _deliverTokens( address _beneficiary, uint256 _tokenAmount ) internal {
        ...
        require(YourToken(token).mint(_beneficiary,_tokenAmount));
  }
